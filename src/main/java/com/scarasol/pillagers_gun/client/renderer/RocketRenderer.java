...
public class RocketRenderer extends EntityRenderer<AbstractArrow> {
    private static final ResourceLocation texture = new ResourceLocation("pillagers_gun:textures/entities/rocket.png");
    private final RocketModel model;

    public RocketRenderer(EntityRendererProvider.Context context) {
        super(context);
        model = new RocketModel(context.bakeLayer(RocketModel.LAYER_LOCATION));
    }

    @Override
    public void render(AbstractArrow entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        model.renderToBuffer(poseStack, vb, packedLightIn, OverlayTexture.NO_OVERLAY, 1, 1, 1, 1f);
